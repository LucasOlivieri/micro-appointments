    ...
    def display_type(self) -> str:
        ...
        for prefix in ("INT", "INTEGER", "TINYINT", "SMALLINT", "MEDIUMINT", "BIGINT"):
            if t.startswith(prefix):
                return "INTEGER"
        if any(
            t.startswith(p) for p in ("REAL", "DOUBLE", "FLOAT", "NUMERIC", "DECIMAL")
        ):
            return "REAL"
        if "BLOB" in t:
            return "BLOB"
...
class Database:
    ...
    __EXCLUDE_TABLES = ["superusers", "schema_migrations"]
    ...
    def get_tables(self) -> list[TableInfo]:
        with self._connect() as conn:
            ...
            for row in cur.fetchall():
                name = row["name"]
                if name in self.__EXCLUDE_TABLES:
                    continue
        ...
        with self._connect() as conn:
            ...
            if search:
                text_cols = [
                    c
                    for c in cols_info
                    if "INT" not in c.type.upper() and "REAL" not in c.type.upper()
                ]
                if text_cols:
                    search_parts = [
            ...
            for col, val in filters.items():
                if col in col_names and val:
                    where_clauses.append(
                        f"CAST({_safe_identifier(col)} AS TEXT) LIKE ?"
                    )
                    params.append(f"%{val}%")
