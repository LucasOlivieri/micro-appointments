    ...
    if _URL_RE.match(raw):
        ...
        if _IMAGE_EXT.search(raw):
            return (
                ...
                f"     onerror=\"this.parentElement.parentElement.innerHTML='<a href"
                f"=&quot;{url}&quot; target=&quot;_blank&quot;>{url}</a>'\" />"
                f"</a>"
                f"</span>"
            )
        ...
        if _FILE_EXT.search(raw):
            ...
            return (
                f'<a href="{url}" class="file-link" target="_blank" rel="noopener">'
                f"📎 {_escape(fname)}</a>"
            )
        ...
        return (
            f'<a href="{url}" class="url-link" target="_blank" '
            f'rel="noopener">{_escape(display)}</a>'
        )
