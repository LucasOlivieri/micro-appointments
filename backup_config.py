...
Exports the current database state back into config.json format with a timestamped filename.
...
def export_config_from_db() -> dict[str, Any]:
    ...
    Returns:
        dict: Configuration with nested users, rules, appointment_types, and blocked_times
    """
    db = get_db()
    ...
    for user in users:
        ...
        rules = [
            row for row in db["rules"].rows
            if row.get("user") == user_id
        ]
        appointment_types = [
            row for row in db["appointment_types"].rows
            if row.get("user") == user_id
        ]
        blocked_times = [
            row for row in db["blocked_times"].rows
            if row.get("user") == user_id
        ]
...
def backup_config(
    ...
    if verbose:
        print(f"Exporting config from database...")
    ...
    num_types = sum(len(u.get("appointment_types", [])) for u in config.get("users", []))
